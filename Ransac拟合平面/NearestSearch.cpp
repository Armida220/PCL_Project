...
//RNN点云邻域搜索
int NearestSearch::nearestRSearch(new3s_PointXYZ &query_point , float radius , std::vector<new3s_PointXYZ> &indice_points , std::vector<float> &distance)
{
	if (m_num_of_cloud == 0)
	{
#ifdef _DEBUG
	...
	int neighbors_in_radius = m_index->radiusSearch(queryset , radius_indices , radius_dists , static_cast<float>(radius * radius) , flann::SearchParams(15)) ;
...
int NearestSearch::nearestRSearch(new3s_PointXYZ &query_point , float radius , std::vector<int> &indices , std::vector<float> &distance)
{
	if (m_num_of_cloud == 0)
	{
#ifdef _DEBUG
	...
	int neighbors_in_radius = m_index->radiusSearch(queryset , radius_indices , radius_dists , static_cast<double>(radius * radius) , flann::SearchParams(15)) ;
